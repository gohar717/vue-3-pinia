...
export const useProductsStore = defineStore('products', (): State => {
    const search = ref('');
    ...
    const productsFilter = computed(() => products.value.filter(e => e.title.toLocaleLowerCase().startsWith(search.value.toLocaleLowerCase())))

    return { products, productsFilter, search }
})
